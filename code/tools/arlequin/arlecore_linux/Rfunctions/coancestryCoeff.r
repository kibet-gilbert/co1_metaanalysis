################################################################################
# coancestryCoeff function  -  (matrix)
#
...
coancestryCoeff <- function(xmlText, Labels=NULL, timeAttr, outfile=outfiles) {
  ...
  # draw matrix ------------------------
  outfileGraphic <- paste(outfile, "coancestryCoeff ", timeAttr,".png", sep="")  
  ...
  png(outfileGraphic, width=1300, height=1300, res=144)
    ...
    old.par <- par(no.readonly = TRUE)
      ...
      image(1, y, z, col = ColorRamp(64),xlab="", ylab="", axes=FALSE)
          ...
          box()
          mtext(text=expression(bold(t/N == -ln(1-F[ST]))), side=4, line=2.5,
                cex=1.1)
      ...
      image(x,y,Matrix, col=ColorRamp(64), xlab="", ylab="",
             main="Matrix of  coancestry coefficients", axes=FALSE)
          box()
