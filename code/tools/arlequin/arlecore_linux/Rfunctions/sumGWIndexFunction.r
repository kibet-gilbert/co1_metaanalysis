################################################################################
# sumGWIndexFunction function  -  (lines)
#
...
sumGWIndexFunction <- function(xmlText,timeAttr, outfile=outfiles){ 
  ...
  outfileGraphic <- paste(outfile, "sumGWIndex ", timeAttr, ".png", sep="")
    ...
    #draw graphic for each pop---------------
      ...
      for(i in 2:ncol(numericMatrix)){
        ...
        if((nRowGraphic == 1 && Nrow < 88) || (nRowGraphic == 2 && Nrow < 48) ||
            ...
            mtext("Garza-Williamson index", side=2 , line=2, cex=labSize)
      ...
      title("Garza-Williamson index at different loci", outer=TRUE, cex.main=1.5)
