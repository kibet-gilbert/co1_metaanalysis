################################################################################
# tauMatrixFunction function  -  (matrix)
#
...
tauMatrixFunction <- function(xmlText, Labels=NULL, timeAttr,outfile=outfiles) {
  ...
  tagMatrix  <- gsub("tau values", "", tagMatrix)
  tagMatrix  <- gsub("----------", "", tagMatrix)
  tagMatrix <- subset(tagMatrix, tagMatrix[,1] != "") #trim empty lines
  tagMatrix <- tagMatrix[2:(nrow(tagMatrix)-1)]
  ...
  # draw matrix plot --------------------
  outfileGraphic <- paste(outfile, "tauMatrixFunction ", timeAttr, ".png",
                            sep="")  
  ...
  png(outfileGraphic, width=1300, height=1300, res=144)
    ...
    old.par <- par(no.readonly = TRUE)
      ...
      image(1, y, z, col = ColorRamp(64),xlab="", ylab="", axes=FALSE)
          ...
          box()
          mtext(text=expression(bold(tau)), side=4, line=2.5, cex=1.5)
      ...
      image(x,y,Matrix, col=ColorRamp(64),
                main="Divergence time between populations", xlab="", ylab="",
                axes=FALSE)
          box()
