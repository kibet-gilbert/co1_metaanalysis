################################################################################
# sumAllelicSizeRangeFunction function  -  (lines)
#
...
sumAllelicSizeRangeFunction <- function(xmlText,timeAttr, outfile=outfiles){ 
  ...
  outfileGraphic <- paste(outfile, "sumAllelicSizeRange ", timeAttr, ".png",
                           sep="")
  ...
  png(outfileGraphic, width=1300, height=1300, res=144)
    ...
    op <- par(oma=c(0,0,2,0), mfrow=c(nRowGraphic, nRowGraphic))
      ...
      for(i in 2:ncol(numericMatrix)){
       ...
       title("Allele size range at different loci", outer=TRUE, cex.main=1.5)
