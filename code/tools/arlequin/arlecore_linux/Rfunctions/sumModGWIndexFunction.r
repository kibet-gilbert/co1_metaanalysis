################################################################################
# sumModGWIndexFunction function  -  (lines)
#
...
sumModGWIndexFunction <- function(xmlText,timeAttr, outfile=outfiles){ 
  ...
  if(Row >= 2){
    for(n in 2:(Row)){
      ...
      nextrow <- nextrow[1:(nrow(nextrow)-2)]
  ...
  outfileGraphic <- paste(outfile, "sumModGWIndex ", timeAttr, ".png", sep="")
    ...
    op <- par(oma=c(0,0,2,0), mfrow=c(nRowGraphic, nRowGraphic))
      ...
      for(i in 2:ncol(numericMatrix)){
        ...
        if((nRowGraphic == 1 && Nrow < 88) || (nRowGraphic == 2 && Nrow < 48) ||
            ...
            mtext("Modified Garza-Williamson index", side=2 , line=2, cex=labSize-0.05)
       ...
       title("Modified Garza-Williamson index at different loci", outer=TRUE
                , cex.main=1.5)
