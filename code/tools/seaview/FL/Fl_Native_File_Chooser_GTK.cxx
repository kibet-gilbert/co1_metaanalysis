...
int mgFl_GTK_File_Chooser::did_find_GTK_libs = 0;
...
mgFl_GTK_File_Chooser::mgFl_GTK_File_Chooser(int val) : mgFl_FLTK_File_Chooser(-1)
{
...
mgFl_GTK_File_Chooser::~mgFl_GTK_File_Chooser()
{
...
void mgFl_GTK_File_Chooser::type(int val) {
  _btype = val;
}

int mgFl_GTK_File_Chooser::count() const {
  return gtkw_count;
}

const char *mgFl_GTK_File_Chooser::filename() const
{
  if(gtkw_ptr) {
...
const char *mgFl_GTK_File_Chooser::filename(int i) const
{
  if(fl_gtk_file_chooser_get_select_multiple((GtkFileChooser *)gtkw_ptr) == FALSE) {
    return gtkw_filename;
  }
...
void mgFl_GTK_File_Chooser::title(const char *val)
{
  strfree(gtkw_title);
  gtkw_title = strnew(val);
}

const char* mgFl_GTK_File_Chooser::title() const
{
  return gtkw_title;
}

/* changes the extension of the outfile in the chooser according to newly selected filter */
void mgFl_GTK_File_Chooser::changed_output_type(const char *filter)
{
...
gboolean mgFl_GTK_File_Chooser::custom_gtk_filter_function(const GtkFileFilterInfo *info, mgFl_GTK_File_Chooser::pair* p)
{
...
void mgFl_GTK_File_Chooser::free_pair(mgFl_GTK_File_Chooser::pair *p)
{
  delete p;
}
...
int mgFl_GTK_File_Chooser::show()
{
...
int mgFl_GTK_File_Chooser::fl_gtk_chooser_wrapper()
{
  int result = 1;
  ...
  if (_parsedfilt) {
    ...
    while (p) {
      ...
      fl_gtk_file_filter_add_custom(filter_tab[count], 
				    ...
				    (GtkFileFilterFunc)mgFl_GTK_File_Chooser::custom_gtk_filter_function, 
				    new mgFl_GTK_File_Chooser::pair(this, p), 
				    (GDestroyNotify)mgFl_GTK_File_Chooser::free_pair);
  ...
  while (response_id == GTK_RESPONSE_NONE) { // loop that shows the GTK dialog window
    ...
    while (XEventsQueued(fl_display, QueuedAfterReading)) { // emulate modal dialog
      ...
      if (!fl_find(xevent.xany.window)) continue; // skip events to non-FLTK windows
      if ( xevent.type == Expose ) fl_handle(xevent); // ignore non Expose events
    }
 ...
 */
void mgFl_GTK_File_Chooser::probe_for_GTK_libs(void) {
  void *ptr_glib    = NULL;
  void *ptr_gtk     = NULL;
