...
bold_data = subset(bold_dataframe, !is.na(nucleotides) & class_name == "Insecta") ## filtering records corresponding to COI-5P markers
bold_data %>% mutate(seqlen1 = nchar(nucleotides)) -> resulting_dataframe1 ## Introducing a field "seqlen1" that has the number of nucleotides in the COI_data
unique(unlist(strsplit(bold_dataframe$nucleotides, "", fixed = TRUE)), incomparables = FALSE) ## list all characters present in the nucleotide sequences
...
unique(resulting_dataframe3$markercode) ## identify and outputs the markers present in the data
COI_data = subset(resulting_dataframe3, markercode == "COI-5P" & !is.na(nucleotides) & class_name == "Insecta") ## filtering records corresponding to COI-5P markers
non_COI_data = subset(resulting_dataframe3, markercode != "COI-5P" & !is.na(nucleotides))
cat("COI-5P marker sequences are ",nrow(COI_data)," out of ", nrow(resulting_dataframe3), "sequences in the raw bold data") ## comparing the number of records before and after removing other markers

###Generating a file with all 'COI-5P' sequences
COI_data -> COI_all_data; cat("\t",length(COI_all_data$unaligned_nucleotides),"sequences have 'COI-5P' marker\n")
...
datalist = lapply(c("COI_all_data", "non_COI_data", "COI_Over499_data", "COI_500to700_data", "COI_650to660_data", "COI_Over700_data", "COI_Under500_data", "COI_testa00_data", "COI_testb01_data", "COI_testb02_data", "COI_testb03_data", "COI_testc04_data", "COI_testc05_data", "COI_testd06_data", "COI_teste07_data"), get)
names(datalist) <- (c("../data/input/test_data/COI_all_data", "../data/input/test_data/NonCOI_data", "../data/input/test_data/COI_Over499_data", "../data/input/test_data/COI_500to700_data", "../data/input/test_data/COI_650to660_data", "../data/input/test_data/COI_Over700_data", "../data/input/test_data/COI_Under500_data", "../data/input/test_data/COI_testa00_data", "../data/input/test_data/COI_testb01_data", "../data/input/test_data/COI_testb02_data", "../data/input/test_data/COI_testb03_data", "../data/input/test_data/COI_testc04_data", "../data/input/test_data/COI_testc05_data", "../data/input/test_data/COI_testd06_data", "../data/input/test_data/COI_teste07_data"))
